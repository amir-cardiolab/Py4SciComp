...
file_conc_output = XDMFFile(results_dir + 'RT.xdmf') # the output name for your results.
...
solution_save_interval = 200 #how often (how many time-steps) do you want to save the results
basis_order = 1 #1st order or 2nd baseis order. Start with 1 and then also try 2.

D = 5e-4 #cm^2/s #Diff coeff. In cardiovascular mass transport typically between 1e-4 cm^2/s - 1e-6 cm^2s. The higher it is the easier the code converge; if your unit is not cm, make sure you CONVERT!!!!
...
F = v * (c - c_prev) / dt * dx \
    ...
    + D_multiple * dot(grad(v), grad(c_mid)) * dx - 1.0 * v * dx 
