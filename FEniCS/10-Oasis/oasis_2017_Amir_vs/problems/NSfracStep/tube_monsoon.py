...
if restart_folder:
        ...
        NS_parameters.update(
            ...
            T= 1.053*2, #0.84
            dt=0.001053 /1.,
            folder= '../../../../scratch/aa3878/tube_oasis/NewtO2_plug',#'../nonNewtonian/AAA18/results_oasis/O1/nonNewt'
            velocity_degree=1,
            save_step=40/2,
            save_start= 2000/2,#4000,
            checkpoint=100*1e9,
            ...
            beta_backflow = 0.05,#0.2, #1.0 , # btwn 0 and 1 (actually beta=2.0 and higher works!)
...
def create_bcs(facet_domains,t,t_cycle,Time_array,v_IR_array, V, Q, mesh, **NS_namespace):
    ...
    if (parabolic_flag):
        ...
        v_inlet_BC = numpy.interp(t_cycle, Time_array, v_IR_array)
        v_inlet_BC = v_inlet_BC / A
        inflow = Expression('-vel_IR',vel_IR= v_inlet_BC,degree=3)
