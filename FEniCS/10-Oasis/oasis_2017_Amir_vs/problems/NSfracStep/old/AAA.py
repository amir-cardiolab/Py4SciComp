...
# Create a mesh
def mesh(**params):
    m = Mesh('/Users/aa3878/data/surface_adv/AAA18/3D_mesh/AAA18_mesh/P18actext_longer-mesh-complete/mesh-surfaces/P18vel_mesh.xml')
...
if restart_folder:
        ...
        NS_parameters.update(
            ...
            T= 0.1, #0.84
            dt=0.001 * 2,
            folder= 'AAA_results_nonNewtonian', #'AAA_results_backflow',
            velocity_degree=1,
            save_step=25 - 12 ,
            checkpoint=50,
            ...
            beta_backflow = 2.0 , # btwn 0 and 1 (actually beta=2.0 and higher works!)
...
def create_bcs(t,t_cycle,Time_array,v_IR_array, V, Q, mesh, **NS_namespace):
    # Specify boundary conditions
    
    bc_file =  '/Users/aa3878/data/surface_adv/AAA18/3D_mesh/AAA18_mesh/P18actext_longer-mesh-complete/mesh-surfaces/BCnodeFacets.xml' #specify it in pre_solve_hook function below too.
    ...
    if (parabolic_flag):
     ...
     if(1): #This works!! (from Womersley.py)
        ...
        # Compute barycenter by integrating x components over all facets
         ...
         normal = ni/n_len
         if(0):
          bc_u0 = Expression("n0 * u * (1 - (pow(cent0-x[0], 2) + pow(cent1-x[1], 2) + pow(cent2-x[2], 2)   ) / r)",n0=normal[0], r=inlet_radius, u=-2.0*v_inlet_BC, cent0=center[0],cent1=center[1],cent2=center[2], degree=3)
          bc_u1 = Expression("n0 * u * (1 - (pow(cent0-x[0], 2) + pow(cent1-x[1], 2)+ pow(cent2-x[2], 2)  ) / r)",n0=normal[1], r=inlet_radius, u=-2.0*v_inlet_BC, cent0=center[0],cent1=center[1],cent2=center[2], degree=3)
          bc_u2 = Expression("n0 * u * (1 - (pow(cent0-x[0], 2) + pow(cent1-x[1], 2)+ pow(cent2-x[2], 2) ) / r)",n0=normal[2], r=inlet_radius, u=-2.0*v_inlet_BC, cent0=center[0],cent1=center[1],cent2=center[2], degree=3)
         else:#we know that the inlet face is in Z direction
           bc_u0 = Expression("n0 * u * (1 - (pow(cent0-x[0], 2) + pow(cent1-x[1], 2) ) / r)",n0=normal[0], r=inlet_radius, u=-2.0*v_inlet_BC, cent0=center[0],cent1=center[1],cent2=center[2], degree=3)
           bc_u1 = Expression("n0 * u * (1 - (pow(cent0-x[0], 2) + pow(cent1-x[1], 2) ) / r)",n0=normal[1], r=inlet_radius, u=-2.0*v_inlet_BC, cent0=center[0],cent1=center[1],cent2=center[2], degree=3)
           bc_u2 = Expression("n0 * u * (1 - (pow(cent0-x[0], 2) + pow(cent1-x[1], 2) ) / r)",n0=normal[2], r=inlet_radius, u=-2.0*v_inlet_BC, cent0=center[0],cent1=center[1],cent2=center[2], degree=3)
    if (parabolic_flag):
...
def pre_solve_hook(mesh, velocity_degree, u_,
    ...
    bc_file =  '/Users/aa3878/data/surface_adv/AAA18/3D_mesh/AAA18_mesh/P18actext_longer-mesh-complete/mesh-surfaces/BCnodeFacets.xml'
    facet_domains = MeshFunction('size_t', mesh,bc_file )
    #normal
    n_normal = FacetNormal(mesh)
