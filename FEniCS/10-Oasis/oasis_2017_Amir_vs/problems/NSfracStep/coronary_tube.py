...
def mesh(**params):
    ...
    m = Mesh('/Users/aa3878/data/stenosis_ALE/tube_mesh/temp_mesh/tube_open_geo_openflipper-mesh-complete/mesh-surfaces/tube_mesh.xml')
    return m
...
if restart_folder:
        ...
        NS_parameters.update(
            ...
            nu=4.0/1.06, #mg/ mm^3 and mg /mm^1 / s^1
            T= 0.04 - 0.01, #0.84
            dt=0.001/2., #assume Q = 500 mm^3/s => v = 131.6 mm/s (dt=0.001 works for O1)
            folder= '../stenosis_ALE/coronary/test_oasis',
            velocity_degree=2,
            save_step=1e9, #save in the code
...
def create_bcs(facet_domains,t,t_cycle,Time_array,v_IR_array, V, Q, mesh, **NS_namespace):
    ...
    inlet_facet_ID = 1
    Inlet_flow_rate = 500.0 #steady flow rate (mm^3/s)
    if (parabolic_flag):
     if(0): #This does not work (See below)
        ...
        #for i in range(3):
        #    #center.append(assemble(p[i]*cd ../1), mesh, facet_domains)/Area)
    ...
    if (parabolic_flag):
        ...
        else: #parabolic profile created from Create_inlet_BC_xml.py
          V_inletBC_x = Function(V,'/Users/aa3878/data/stenosis_ALE/tube_mesh/BC/bctxml_x.xml')
          V_inletBC_y = Function(V,'/Users/aa3878/data/stenosis_ALE/tube_mesh/BC/bctxml_y.xml')
          V_inletBC_z = Function(V,'/Users/aa3878/data/stenosis_ALE/tube_mesh/BC/bctxml_z.xml')
          bc_in_1  = DirichletBC(V, V_inletBC_x , facet_domains,inlet_facet_ID) #inlet for u_x
          bc_in_2  = DirichletBC(V, V_inletBC_y , facet_domains,inlet_facet_ID) #inlet for u_y
          bc_in_3  = DirichletBC(V, V_inletBC_z, facet_domains,inlet_facet_ID) #inlet for u_z
    else:
...
def pre_solve_hook(jj,mesh, velocity_degree, u_,
                   AssignedVectorFunction, **NS_namespace):
    #bc_file =   '/scratch/aa3878/tube_oasis/BCnodeFacets.xml'
    if (jj==0):
      #bc_file =  '/Users/aa3878/data/stenosis_ALE/tube_mesh/mesh-complete-mesh-complete/mesh-surfaces/BCnodeFacets.xml'
      bc_file = '/Users/aa3878/data/stenosis_ALE/tube_mesh/temp_mesh/tube_open_geo_openflipper-mesh-complete/mesh-surfaces/BCnodeFacets.xml'
