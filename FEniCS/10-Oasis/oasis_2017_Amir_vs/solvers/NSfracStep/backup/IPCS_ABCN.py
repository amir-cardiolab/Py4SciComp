...
def setup(u_components,backflow_flag,beta_backflow,Resistance_flag,n_normal,facet_domains, u, v, p, q, bcs, les_model, nu, nut_,
    ...
    if(nonNewtonian_flag and backflow_flag):
        ...
        ds = Measure("ds")[facet_domains]
        K = lhs( visc_nu*inner(grad(u), grad(v)) * dx - 2.0/nu*beta_backflow *  inner(v, ( dot(u_,n_normal) - abs( dot(u_,n_normal) ) )/2. * u ) * (ds(3, domain=mesh, subdomain_data=facet_domains) + ds(4, domain=mesh, subdomain_data=facet_domains) )   )
    ...
    d.update(u_ab=u_ab,u_ab_prev=u_ab_prev, a_conv=a_conv, a_scalar=a_scalar, LT=LT, KT=KT,K=K)
    return d
...
def assemble_first_inner_iter(nonNewtonian_flag,backflow_flag, beta_backflow,mesh,facet_domains,n_normal, A, a_conv, dt, M, scalar_components, les_model,
    ...
    if(nonNewtonian_flag):
     ...
     K_weak = visc_nu * inner(grad(u), grad(v)) * dx #need to assemble again due to viscossity change
     if(backflow_flag):
          K_weak = K_weak - 2.0*beta_backflow *  inner(v, ( dot(u_,n_normal) - abs( dot(u_,n_normal) ) )/2. * u ) * (ds(3) + ds(4)) #or ds(3,domain=mesh,subdomain_data=facet_domains) #factor 2, because axpy below multiplies by 0.5
          K_weak_lhs = lhs(K_weak)
          K = assemble_matrix(K_weak_lhs)
     else:
        K = assemble_matrix(K_weak)
     A.axpy(-0.5, K, True)
    else:
      if(backflow_flag):
             ...
             A.axpy(-0.5*nu, K2, True) #A.axpy(-0.5*nu, K, True)
      else:
...
def pressure_solve(Resistance_flag,Res1, Res2,Q,n_normal,facet_domains,mesh,assemble_matrix,u_,dp_, x_, Ap,Ap2, b, p_sol, bcs, **NS_namespace):
    ...
    if (Resistance_flag):
       ...
       print '+++++++++++++++++++++++Res_bc: 1/2ratio:', flow1/flow2
       bcs['p'] = [DirichletBC(Q,  Res_bc1 , facet_domains,3), DirichletBC(Q,  Res_bc2, facet_domains,4)]
       #Ap = assemble_matrix(Ap2, bcs['p'])
