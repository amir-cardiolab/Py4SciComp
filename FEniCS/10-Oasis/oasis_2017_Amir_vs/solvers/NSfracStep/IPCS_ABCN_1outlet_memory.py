...
#IPCS_ABCN but with one outlet (for printing flowrate at the outlet)
#trying to prevent memory leak in the setup function.
# THIS FUNCTION WAS FOR TESTING MEMORY. Use IPCS_ABC_1outlet.py instead. 
...
def setup(jj,u_components,backflow_flag,nonNewtonian_flag, beta_backflow,Resistance_flag,n_normal,facet_domains, u, v, p, q, bcs, les_model, nu, nut_,
    ...
    A = Matrix(M)
    #A = []
    if (1): # the main cause of memory leak! divu and gradp (gradp more)
    ...
    # name2 = 'divu' + str (kk)
    # print kk
    # divu = DivFunction(u_, Q, name=name2,method=velocity_update_solver)
     #del divu.form,divu.method, divu.bcs,divu.matvec,divu.trial,divu.test,divu.bf,divu.rhs,divu.A,divu.sol  # does not reduce memory!
     #gc.collect()
     #del divu
     #divu=[]
    ...
    #d = dict(A=A, M=M, Ap=Ap,Ap2=Ap2) #temp! Amir!
    ...
    if(nonNewtonian_flag and backflow_flag):
        ...
        ds = Measure("ds")[facet_domains]
        K = lhs( visc_nu*inner(grad(u), grad(v)) * dx - 2.0*beta_backflow *  inner(v, ( dot(u_,n_normal) - abs( dot(u_,n_normal) ) )/2. * u ) * (ds(3, domain=mesh, subdomain_data=facet_domains)) )
         ...
         #gc.collect()
         ds = Measure("ds")[facet_domains]
         K = lhs( nu*inner(grad(u), grad(v)) * dx - 2.0*beta_backflow *  inner(v, ( dot(u_,n_normal) - abs( dot(u_,n_normal) ) )/2. * u ) * (ds(3, domain=mesh, subdomain_data=facet_domains)))
         #K = lhs( nu*inner(grad(u), grad(v)) * dx - 2.0*beta_backflow *  inner(v, ( dot(u_ab,n_normal) - abs( dot(u_ab,n_normal) ) )/2. * u ) * (ds(3, domain=mesh, subdomain_data=facet_domains)) )
         visc_nu = nu
    else:
         if (1):
          K = assemble_matrix(inner(grad(u), grad(v)) * dx)
          ...
          K = []
          mytemp = assemble_matrix(inner(grad(u), grad(v)) * dx) #This will cause leak!!
          del mytemp
         visc_nu = nu
...
def pressure_solve(Resistance_flag,Res1, Res2,Q,n_normal,facet_domains,mesh,assemble_matrix,u_,dp_, x_, Ap,Ap2, b, p_sol, bcs, **NS_namespace):
    ...
    if (Resistance_flag):
       ...
       #bcs['p'] = [DirichletBC(Q,  Res_bc1 , facet_domains,3), DirichletBC(Q,  Res_bc2, facet_domains,4)]
       bcs['p'] = [DirichletBC(Q,  Res_bc1 , facet_domains,3)]
       #Ap = assemble_matrix(Ap2, bcs['p'])
...
def velocity_update(u_components, bcs, gradp, dp_, dt, x_, **NS_namespace):
    ...
    for ui in u_components:
        print 'entering'
        gradp[ui](dp_) #memory leak!
        print 'memoV1', resource.getrusage(resource.RUSAGE_SELF).ru_maxrss * 1e-6
        x_[ui].axpy(-dt, gradp[ui].vector())
        print 'memoV2', resource.getrusage(resource.RUSAGE_SELF).ru_maxrss * 1e-6
        [bc.apply(x_[ui]) for bc in bcs[ui]]
        print 'memoV3', resource.getrusage(resource.RUSAGE_SELF).ru_maxrss * 1e-6
    #del dp_
    #gc.collect()
